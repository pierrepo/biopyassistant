...
def get_args() -> Tuple[str, str, str]:
    ...
    # Create the parser
    parser = argparse.ArgumentParser(description="Save details of chunks to a text file and save the number of tokens and chunks for each files to a CSV file.")
    
    parser.add_argument("--data_dir", type=str, required=True, help="The path to the directory containing the Markdown documents.")
    parser.add_argument("--chroma_path", type=str, required=True, help="The path to the directory containing the Chroma database.")
    parser.add_argument("--txt_output", type=str, default=None, help="The name of the output file to save the text chunks with metadata.")
    parser.add_argument("--csv_output", type=str, default=None, help="The name of the output file to save number of tokens and chunks for each Markdown files contained in the data directory.")
    ...
    if not os.path.exists(args.data_dir):
        logger.error(f"The directory '{args.data_dir}' specified by --data_dir does not exist.")
        sys.exit(1) # Exit the program
    if not os.path.exists(args.chroma_path):
        logger.error(f"The directory '{args.chroma_path}' specified by --chroma_path does not exist.")
        sys.exit(1) # Exit the program

    # Warnings if none of the output files are specified
    if args.txt_output == None and args.csv_output == None:
        logger.error("No output file specified. Please specify at least one output file.")
        sys.exit(1) # Exit the program

    # Log information for output files
    if args.txt_output != None:
        logger.info(f"The details of each chunk will be saved in {args.txt_output}.txt.")
    if args.csv_output != None:
        logger.info(f"The number of tokens and chunks for each Markdown file will be saved in {args.csv_output}.csv.")
...
def add_file_names_to_metadata(documents: List[Document]) -> List[Document]:
    ...
    logger.success("Added the file names in the metadata of the documents successfully.\n")
...
def add__nb_tokens_to_metadata(documents: List[Document]) -> List[Document]:
    ...
    logger.success("Added the number of tokens in the metadata of the documents successfully.\n")
...
def reconstruct_chunks(vector_db: Chroma) -> List[Document]:
    ...
    logger.success(f"Reconstructed {total_chunks} chunks from the vector database successfully.\n")

    return chunks


def save_to_txt(
    chunks: List[Document], txt_output: str) -> None:
...
def save_to_csv(file_names: List[str], documents: List[Document], chunks: List[Document], csv_output: str) -> None:
        ...
        for filename in file_names:
            ...
            for chunk in chunks:
                if chunk.metadata.get("file_name", "") == filename:
                    ...
                    chunk_number += 1
            f.write(f"{filename}, {token_number}, {chunk_number}, {token_number_from_chunks}\n")
